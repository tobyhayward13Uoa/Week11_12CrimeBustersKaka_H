...
prosecution.df = filter(data, `Police Action` == "Prosecution")
prosecution.df
```
...
```{R}
library(data.table)
data_v2 <- setDT(prosecution.df)[ , .(Clearances_month = sum(Clearances)), by = Month]
data_v2
```
...
General plot shows increasing trend, with a massive drop in May 2020. Variance fairly constant?

Seasonal plot shows that possibly number of prosecutions is highest at the middle of the year
...
ggplot(aes(Month, Clearances_month))+geom_line()+geom_area(fill = scales::hue_pal()(1), alpha = 0.4) +
  bbc_style() +
  labs(title = 'Prosecutions over Time',
       x = 'Time', y = 'Prosecutions') +
...
gg_season(Clearances_month, labels = "both") +
  ...
  labs(title = 'Seasonal Variation by Year',
       x = 'Month', y = 'Prosecutions') +
  theme(axis.title = element_text(size = 20))
ggsave('test.png')
```

Variability is  nonconstant as level increases, so  transform.
...
AICc is really the main information criteria- lower AICc=better model. In this case our additive (same as automated ets model) performs best with the lowest AICc.
...
All 4 are measures of error- lower=better. Additive_Damped performs best by far. Overall, we will go with a additive_damped model, as it has second best model fit and best forecast accuracy.
...
ACF plot shows 7 significant lags and 1 significant seasonal lag (at 12). Suggests an ARIMA(0,0,7)(0,1,1)[12] would be suitable.
...
```{R}
fit <- training %>%
  model(ARIMA007011 = ARIMA(box_cox(Clearances_month, lambda) ~ pdq(0, 0, 7) + PDQ(0,1,1)),
...
Stepwise function recommends <ARIMA(2,0,0)(0,1,0)[12]
Search function recommends <ARIMA(0,0,2)(0,1,1)[12] **Note: knit the document to see this!**
...
Stepwise and search all have similar AICc values.
...
ARIMA100110 performs best in terms of accuracy to test data. Let's take search as it has best model fit + decent accuracy- ARIMA(0,0,2)(0,1,1)[12]
...
Therefore, the fitted model for ARIMA(0,0,2)(0,1,1)[12] is:

$y_t = 44.6 + 0.31\epsilon_{t-1} + 0.46\epsilon_{t-2} - 0.58\epsilon_{t-12} + \epsilon_t$ , where $\epsilon_t$ is estimated to be a white noise series with mean 0 and a variance of 1928.

We can now produce forecasts for post-MODA changes, and compare to the actual data.

```{R}
best_fc <- pre_moda %>%
  model(ARIMA002011 = ARIMA(box_cox(Clearances_month, lambda) ~ pdq(0, 0, 2) + PDQ(0,1,1))) %>%
  forecast(h = 24)
# best_fc %>%
...
Clear difference- significant change!
...
That's statistical significance.
...
Disribution looks roughly normal. Lets keep to the definition and apply the transformation.
...
Looks like a perfect fit! Now we can use it to simulate some data!
...
hist(rss_sim[rss_sim < 2e7], xlim = c(0, 3e6))
...
Clear difference! Very unlikely.
